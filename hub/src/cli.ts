...
//   claude-web --root DIR      sessions under DIR instead of the cwd
...
export function parseArgs(argv: string[], env: NodeJS.ProcessEnv = process.env): CliArgs {
  ...
  for (let i = 0; i < argv.length; i++) {
    ...
    if (a === "--port" || a === "-p") args.port = Number(next());
      ...
      console.log(
        "usage: claude-web [--port N] [--host H] [--root DIR] [--open]\n" +
          "Serve the browser UI for the Claude Code sessions under DIR (default: cwd).",
      );
      process.exit(0);
...
if (import.meta.main) {
  ...
  console.log(`claude-web: ${url}  (sessions under ${args.root})`);
  if (args.open) {
